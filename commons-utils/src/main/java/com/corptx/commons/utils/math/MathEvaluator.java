 ...
 * @date     April 2001
 * @author <a href="mailto:widipa@gmail.com.com">William Diaz Pabón</a>, 
 * Adiciono la posibilidad de soportar números con notación cientifica.
 * <pre>
 * Sample:
 * MathEvaluator m = new MathEvaluator("(1225000.00+4000.00)*((1+6.7613E-4)-1)");
 * System.out.println( m.getValue() );
 * la formula: "(1225000.00+4000.00)*((1+6.7613E-4)-1)"
 * la convierta asi: "(1225000.00+4000.00)*((1+0.00067613)-1)"
 * </pre>
 * @date     25/08/2009
 ************************************************************************/
...
//        String formula = "(9.0E7*6.7613E-4)-(6.7613E-4)";
        String formula = "(1+17.28)^(1/12)-1";
            Double var = 8283.51;
             System.out.println("REdondeo: "+ Redondeo.redondear(1.2, 0, Redondeo.ROUND_CEILING));

            System.out.println("Redondeo.redondear(var, 2, Redondeo.ROUND_HALF_EVEN) = " + Redondeo.redondear(var, 0, Redondeo.ROUND_HALF_EVEN));
        try {
    ...
    public void setExpression(String s) {
        ...
        expression = validateScientificNotation(s);
//        System.out.println("expression = " + expression);

    }

    public String validateScientificNotation(String formula) {
...
//            System.out.println("newFormula = " + newFormula);
            ...
            pos = pos == -1 ? newFormula.indexOf("E") : pos;
            pos = pos == -1 ? newFormula.indexOf("e") : pos;
...
//                System.out.println("HACIA ATRAS");
                // Recorre el String hacia atras hasta encontrar un caracter diferente a {'.','E','e','0-9'}
                contador = pos + 1;
                while (contador > 0) {
                    contador--;
//                    System.out.println("contador = " + contador);
//                    System.out.println("newFormula.charAt(contador) = " + newFormula.charAt(contador));
                    if (newFormula.charAt(contador) == '.' ||
                            newFormula.charAt(contador) == 'E' ||
                            newFormula.charAt(contador) == 'e' ||
                            (newFormula.charAt(contador) >= '0' && newFormula.charAt(contador) <= '9')) {
                        strNumero = newFormula.charAt(contador) + strNumero;
//                        System.out.println("strNumero = " + strNumero);
                    } else {
//                        System.out.println("Entro por el break ");
                        break;
                    }
                }

//                System.out.println("HACIA ADELANTE");
                // Recorre el String hacia adelante hasta encontrar un caracter diferente a {'-','E','e','0-9'}
                contador = pos + 1;
                boolean numeroE = false;
                while (contador < newFormula.length()) {
//                    System.out.println("contador = " + contador);
//                    System.out.println("newFormula.charAt(contador) = " + newFormula.charAt(contador));
                    if (newFormula.charAt(contador) >= '0' && newFormula.charAt(contador) <= '9') {
//                        System.out.println("Entro a if (newFormula.charAt(contador) >= '0' && newFormula.charAt(contador) <= '9')");
                        strNumero += newFormula.charAt(contador);
//                        System.out.println("strNumero = " + strNumero);
                        numeroE = true;
                    } else if (!numeroE && (newFormula.charAt(contador) == '-' || newFormula.charAt(contador) == '+')) {
//                        System.out.println("Entro a else if (!numeroE && (newFormula.charAt(contador) == '-' || newFormula.charAt(contador) == '+'))");
                        strNumero += newFormula.charAt(contador);
//                        System.out.println("strNumero = " + strNumero);
                    } else {
//                        System.out.println("Entro por el break ");
...
//                    System.out.println("ENTRO AL TRY");
                    if (!StringUtils.isBlank(strNumero)) {
//                        System.out.println("EL strNumero NO ES BLANK = " + strNumero);
                        BigDecimal numero = new BigDecimal(strNumero);
                        // Este locale es para que los decimales sean separaados por . (punto) y no por , (coma)
                        Locale.setDefault(Locale.US);
                        DecimalFormat df = new DecimalFormat("########.##############################");
                        String newStrNumero = df.format(numero);
//                        System.out.println("newStrNumero = " + newStrNumero);
                        newFormula = StringUtils.replace(newFormula, strNumero, newStrNumero);
//                        System.out.println("newFormula = " + newFormula);
                    } else {
//                        System.out.println("El strNumero ES BLANK");
                        newFormula = formula;
                    }
                } catch (Exception e) {
                    newFormula = formula;
                ...
                // Buscar mas Notaciones cientificas
                pos = -1;
                pos = pos == -1 ? newFormula.indexOf("E") : pos;
                pos = pos == -1 ? newFormula.indexOf("e") : pos;
    ...
    public void trace() {
        try {
            ...
            logError(MathEvaluator.class, "Error en  trace()",e);
        }
    }
